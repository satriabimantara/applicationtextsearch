...
class NFA{
	function delta($fungsi_transisi,$state,$simbol_input){
		//i_row dan i_col untuk mencatat indeks baris dan indeks kolom pada matriks fungsi transisi
		$i_row = null;
		$i_col = null;
	...
	function checkIfCharIsSpecialCharacter($char){
		//sc -> ASCII [33-47],[58-64],[91-96],[123-126]
		//spc -> ASCII 32
		$ascii_char = ord($char);
	...
	function findFungsiTransisi($set_state,$sigma,$start_state,$state_penampung,$final_state,$keywords){
		...
		*/
		...
		//1. Inisialisasi tabel fungsi transisi (banyak state x banyak sigma )
		...
		//2. Membuat mesin NFA otomatis dari setiap keywords
		/*
		...
		*/
		for ($col = 1; $col <= count($sigma) ; $col++) {
			...
			//untuk semua sigma maka menuju ke state penampung
		...
		*/
		...
		for ($idx_keywords = 0; $idx_keywords < count($keywords); $idx_keywords++) {
			//memecah array keywords menjadi setiap keyword
			$keyword = $keywords[$idx_keywords]; 
			...
			$index_kolom = array_search($keyword[0], $sigma)+1;
			array_push($fungsi_transisi[1][$index_kolom],$nama_state);
			...
			for ($idx_keyword = 1; $idx_keyword < strlen($keyword); $idx_keyword++) {
				...
				$index_kolom = array_search($keyword_char, $sigma)+1;
				array_push($fungsi_transisi[$index_start_state][$index_kolom],$nama_state);
				$index_start_state++;
			}
	...
	function main($kata_kunci,$dokumen_uji){
		$keywords = array();
		// Deklarasi variabel return value quintuples
		$quintuples = array();
		...
		//pecah kata kunci dengan delimiter spasi
		...
		*/
		...
		for ($index_string_uji = 0; $index_string_uji < strlen($string_uji); $index_string_uji++) {
			...
			//langkah induksi
			$karakter_dibaca = $string_uji[$index_string_uji];
			$result_set_before =  $this->delta_topi($result_set_before,$karakter_dibaca,$fungsi_transisi);
			//masukkan karakter per karakter ke dalam cuplikan_teks
			array_push($cuplikan_teks, $karakter_dibaca);
